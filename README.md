# Hi there 👋 I'm Javi

🚀 DevOps Engineer from Argentina focused on Kubernetes, OpenShift, CI/CD and cloud-native infrastructure.

I enjoy building scalable platforms, automating workflows and creating real-world infrastructure demos that simulate production problems at scale.

---

## 🛠 Tech Stack

### DevOps & Cloud
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?logo=kubernetes&logoColor=white)
![OpenShift](https://img.shields.io/badge/OpenShift-EE0000?logo=redhatopenshift&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white)
![ArgoCD](https://img.shields.io/badge/ArgoCD-EF7B4D?logo=argo&logoColor=white)
![Tekton](https://img.shields.io/badge/Tekton-FD495C?logo=tekton&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?logo=githubactions&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?logo=linux&logoColor=black)

### Development
- React
- Next.js
- TypeScript
- TailwindCSS
- Node.js

---

## 📌 What I Like Building

- Production-grade Kubernetes demos
- CI/CD pipelines
- GitOps platforms
- Self-hosted infrastructure
- Scalable architectures
- Monitoring & observability stacks
- Real-world failure simulations

---

## 🚧 Current Focus

- Platform Engineering
- Kubernetes Architecture
- Cloud-native resiliency
- High availability systems
- Infrastructure automation

---

## 🚀 Featured Projects

### Black Friday Checkout Collapse
Production-like Kubernetes demo simulating massive checkout failures under high traffic.

### GitOps Platform
ArgoCD + Tekton + Kubernetes CI/CD workflow.
